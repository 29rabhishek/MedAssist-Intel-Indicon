...
import requests
import os
from utils import detect_with_sliding_window, chk_med,  truncate_to_two_sentences
...
async def upload_image(file: UploadFile = File(...)):
    try:
        ...
        dynamic_prompt = f"""Tell me about these medicines and their effects: {" ".join(predictions)} in 30 words"""
...
async def generate_text(request: LlamaRequest):
    ...
    try:
        ...
        payload = {
            "model": "llama3.2:3b",  # Replace with the appropriate model name if different
        ...
        # truncated_text = truncate_to_two_sentences(generated_text)

        # # Return the truncated text
        # return {"response": truncated_text}
